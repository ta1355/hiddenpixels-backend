...
public interface GameRepository extends JpaRepository<Game, Long> {
    ...
    */
    @Query(
            value = "SELECT * FROM game WHERE CAST(REPLACE(price, ',', '') AS DOUBLE) <= :price",
            countQuery = "SELECT count(*) FROM game WHERE CAST(REPLACE(price, ',', '') AS DOUBLE) <= :price",
            nativeQuery = true
    )
    Page<Game> findByPriceLessThanEqual(@Param("price") double price, Pageable pageable);
